...
import { Plus, Filter, Eye, Pencil, Trash2, Calendar, Users, Download, MapPin, Clock } from "lucide-react";
...
export default function EventManagement() {
  ...
  const [nationalOnlyFilter, setNationalOnlyFilter] = useState(false);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {event.affiliate_name && (
              <div className="text-xs text-gray-500">by {event.affiliate_name}</div>
            )}
          </div>
        </div>
      )
  ...
  const fetchEvents = async (
    ...
    try {
      ...
      if (nationalOnlyFilter) params.append("national_only", "true");
  ...
  const fetchEventTypes = async (): Promise<string[]> => {
    try {
      ...
      const token = session.access_token;
      const apiUrl = import.meta.env.VITE_API_BASE_URL;
      const response = await fetch(`${apiUrl}/api/events/types/list`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
  ...
  const fetchAffiliates = async (): Promise<string[]> => {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) throw new Error("No active session");
      ...
      const token = session.access_token;
      const apiUrl = import.meta.env.VITE_API_BASE_URL;
      const response = await fetch(`${apiUrl}/api/events/affiliates/list`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
  ...
  const handleViewAttendance = async (eventId: number) => {
    try {
      ...
      const token = session.access_token;
      const apiUrl = import.meta.env.VITE_API_BASE_URL;
      const response = await fetch(`${apiUrl}/api/events/${eventId}/attendance-report`, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
        },
  ...
  const handleUpdateEvent = async (formData: EventFormData) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/events/${editingEvent.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const refreshTable = () => {
    queryClient.invalidateQueries({
      queryKey: ["events", debouncedSearch, affiliateFilter, eventTypeFilter, statusFilter, startDateFilter, endDateFilter, nationalOnlyFilter],
    });
  };
        ...
        <>
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="mt-1 text-gray-600">Manage national and affiliate events</p>
            </div>
          </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          <label className="flex items-center">
                            <input
                              type="checkbox"
                              checked={nationalOnlyFilter}
                              onChange={(e) => setNationalOnlyFilter(e.target.checked)}
                              className="text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                            />
                            <span className="ml-2 text-sm text-gray-700">National Events Only</span>
                          </label>
                        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div>
          <h3 className="text-lg font-medium text-gray-900">Organizer & Attendance</h3>
          <dl className="mt-4 space-y-3">
            <div>
              <dt className="text-sm font-medium text-gray-500">Organizer</dt>
              <dd className="mt-1 text-sm text-gray-900">
                {event.affiliate_name || (event.is_national_event ? 'National' : 'Not specified')}
              </dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-gray-500">Event Scope</dt>
              <dd className="mt-1 text-sm text-gray-900">
                {event.is_national_event ? 'National Event' : 'Affiliate Event'}
              </dd>
            </div>
            {event.attendance_summary && (
              <>
...
function EventForm({ 
  ...
  const [formData, setFormData] = useState<EventFormData>({
    ...
    affiliate_id: event?.affiliate_id || 0,
    affiliate_name: event?.affiliate_name || "",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => handleChange('is_national_event', e.target.checked)}
                className="text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label htmlFor="affiliate_name" className="block text-sm font-medium text-gray-700">
              Affiliate Name
            </label>
            <input
              type="text"
