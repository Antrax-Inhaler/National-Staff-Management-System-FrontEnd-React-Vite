...
export default function NationalDashboard() {
  ...
  const getMembershipTrendChartData = () => {
    ...
    return {
      ...
      datasets: [
        ...
        {
          label: 'Total Members',
          data: dashboardData.membership_trend.map(item => item.total_members),
          borderColor: 'rgb(59, 130, 246)',
      ...
      datasets: [
        {
          ...
          data: dashboardData.event_engagement.map(item => item.total_attendance),
          backgroundColor: 'rgba(168, 85, 247, 0.8)',
        },
        {
          label: 'Affiliates Represented',
          data: dashboardData.event_engagement.map(item => item.affiliates_represented),
  ...
  const getContractArbitrationChartData = () => {
    ...
    const sortedData = [...dashboardData.contract_arbitration_timeline]
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      <div className="mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">National Administration Dashboard</h1>
          <p className="mt-2 text-gray-600">Comprehensive overview of all affiliates and national activities</p>
        </div>
          ...
          <div className="p-6 bg-white rounded-lg shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-gray-500">
                {dashboardData.compliance_overview.summary.compliance_rate}% compliant
              </span>
            </div>
            <div className="h-80">
              {complianceData ? (
        ...
        <div className="p-6 mb-6 bg-white rounded-lg shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">Contracts vs Arbitrations</span>
          </div>
          <div className="h-80">
            {contractArbitrationData ? (
              <Bar 
                data={contractArbitrationData} 
                options={chartOptions}
              />
            ) : (
              <div className="flex items-center justify-center h-full text-gray-500">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="p-6 bg-white rounded-lg shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr>
                    <th className="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase">Affiliate</th>
                    <th className="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase">Filled Positions</th>
                    <th className="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase">Completion</th>
                    <th className="px-4 py-2 text-xs font-medium text-left text-gray-500 uppercase">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {dashboardData.compliance_overview.detailed_data.slice(0, 5).map((affiliate) => (
                    <tr key={affiliate.affiliate_id}>
                      <td className="px-4 py-2 text-sm">{affiliate.affiliate_name}</td>
                      <td className="px-4 py-2 text-sm">
                        {affiliate.filled_positions}/{affiliate.total_positions}
                      </td>
                      <td className="px-4 py-2 text-sm">{affiliate.completion_rate}%</td>
                      <td className="px-4 py-2 text-sm">
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          ...
                          {affiliate.is_compliant ? 'Compliant' : 'Non-Compliant'}
                        </span>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="p-6 bg-white rounded-lg shadow-sm">
            <h2 className="mb-4 text-xl font-semibold">Recent Event Engagement</h2>
