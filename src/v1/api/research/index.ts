...
import type { DocumentForm, OverviewFilter, UpdateDocumentForm } from "@v1/types";
...
export const research = {
  async overview(params: overview) {
    ...
    const response = await request(`research/overview?${query.toString()}`);
  ...
  async upload(payload: DocumentForm, affiliate_uid?: string) {
    ...
    const response = await request("research/upload", {
      method: "POST",
      body: formData,
      formData: true,
    });
  ...
  async update(payload: UpdateDocumentForm, id: number) {
    const response = await request(`research/update/${id}`, {
      method: "PUT",
  ...
  async delete(id: number) {
    const response = await request(`research/delete/${id}`, {
      method: "DELETE",
    });
  ...
  async createFolder(
    ...
    const response = await request("research/create-folder", {
      method: "POST",
      body: formData,
    });
  ...
  async updateFolder(folder_name: string, folder_uid?: string | null) {
    ...
    const response = await request("research/update-folder", {
      method: "PUT",
      body: formData,
    });
  ...
  async folderTree(search?: string) {
    ...
    const response = await request(
      `research/affiliate-folders?${query.toString()}`,
    );
  ...
  async folders(folder_uid?: string, affiliate_uid?: string) {
    ...
    const res = await request(`research/folders?${params.toString()}`);
    return res.json();
  },

  // New Function ================================================================================

  async newOverview(filters: OverviewFilter) {
    const query = new URLSearchParams();
    ...
    const response = await request(`research/new-overview?${query.toString()}`);
    const result = await response.json();
  ...
  async affiliate(filters: OverviewFilter, affiliate_uid: string) {
    ...
    const response = await request(
      `research/affiliate/${affiliate_uid}?${query.toString()}`,
    );
    const result = await response.json();
  ...
  async index(filters: OverviewFilter) {
    ...
    const response = await request(`research/index?${query.toString()}`);
    const result = await response.json();
  ...
  async arbitrators() {
    const response = await request(`research/arbitrators`);
    if (!response.ok) {
      const result = await response.json();
      if (result.errors) {
        throw result;
