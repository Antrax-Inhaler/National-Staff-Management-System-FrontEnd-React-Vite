// src/components/affiliate/EditAffiliate.tsx
...
import type { Affiliate, AffiliateFormData } from "@v1/types";
...
export default function EditAffiliate({
  ...
  const [open, setOpen] = useState(true); // Always open when component is rendered
  ...
  const [formData, setFormData] = useState<AffiliateFormData>({
    ...
    employer_name: affiliate_data.employer_name ?? "",
    ein: affiliate_data.ein ?? "",
    affiliation_date: affiliate_data.affiliation_date ?? "",
    affiliate_type: affiliate_data.affiliate_type ?? "",
    cbc_region: affiliate_data.cbc_region ?? "",
    ORG_region: affiliate_data.ORG_region ?? "",
  });
  const [logoFile, setLogoFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(affiliate_data.logo_signed_url);
  const [errors, setErrors] = useState<Record<string, string[]>>({});
  const queryClient = useQueryClient();

  useEffect(() => {
    setOpen(true); // Ensure modal is open when component mounts
  }, []);
      ...
      <form onSubmit={handleSubmit} className="bg-white">
        ...
        <div className="mt-5 pt-3 flex justify-end gap-2.5 border-t border-gray-200">
          ...
          <ActionButton
            ...
            label={creating_affiliate ? "Updating..." : "Update Affiliate"}
